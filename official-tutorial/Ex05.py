# Ex05.py
# 210214

# 인자 목록 언패킹

# 인자가 리스트나 튜플에 있지만, 분리된 위치 인자를 요구하는 함수 존재함
# 예를 들어 범위인 range() 내장함수는 시작과 끝 존재함
# 따로 없는 경우, * 연산자로 함수 호출하면 됨

# 뭔말인지 모르겠으니 그냥 계속 따라갑니다

print(list(range(3,6)))
# 분리된 인자를 평범하게 호출함
# 3, 4, 5

args = [3, 6]
print(list(range(*args)))
# 리스트에서 언팩된 인자와 함께 호출함
# 3, 4, 5


# 딕셔너리도 ** 연산자를 써서 키워드 인자 전달가능


def parrot(voltage, state='깡', action='붐'):
    print("-- 이 앵무새는", action, end=' ')
    print('네가 만약', voltage, '볼트를 통과시키면.', end=' ')
    print("얘는", state, '!')


d = {'voltage': '사백만', 'state': '피나요', 'action': '붐--'}
parrot(**d)


# 람다 표현식

# lambda 키워드로 익명함수 만들기
# lambda a, b : a+b 같은 방식으로 사용함
# 함수 객체가 있는 곳이면 어디나 사용가능
# 표현식은 하나만 사용가능(문법상 제한)


def 덧셈기(n):
    """함수를 돌려주기 위해 람다 표현식 사용"""
    print('n =', n)
    return lambda x: x+n


f = 덧셈기(42)
print(f(0))
print(f(30))

print(덧셈기(40)(30))
# 둘러싸는스코프에 있는 변수 참조가능
# 근데 이렇게 써도 되긴 되네 역시
# 메뉴얼은 너무 축약된 것 같은데 아무래도 강의같은 거 들어야하나 미치겠네 정말

# 다른 용도로, 작은 함수를 인자로 전달하기도 함
pairs = [
    (1, '다'),
    (2, '가'),
    (3, '나'),
    (4, '라')
]
pairs.sort(key=lambda pair: pair[1])
print(pairs)


# 도큐멘테이션 문자열
# 1 : 객체의 목적을 짧고 간결하게 요약. 대문자~마침표. 
# 2 : 비어있음. 시각적으로 요약과 나머지 설명 분리. 
# 3 이하 : 객체 호출 규약, 부작용 등 설명

# 파서는 리터럴에서 들여쓰기 제거하지 않음


def 내_함수():
    """문서화를 설명하기 위해서 작성합니다. 
    
    사실, 별 의미 없는 함수에요. 사용해도 그냥 지나가는 함수입니다. 
    """
    pass


print(내_함수.__doc__)


# https://docs.python.org/ko/3/tutorial/controlflow.html#function-annotations